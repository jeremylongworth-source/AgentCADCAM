...
- For execution-adjacent additive jobs, requires `additive_mesh` bytes and schema-checked `setup.additive_preflight` inputs, binds source/job/output and versioned slicer identity, reruns STL/3MF preflight, and requires current design/geometry/orientation/supports/slicer/environment/output evidence. Missing, conflicting or stale evidence invalidates effective approval. See the [additive integration and migration contract](../docs/architecture/additive-router-integration.md).
- For execution-adjacent CAD jobs, requires `cad_artifacts` bytes and schema-checked `setup.cad_handoff` inputs. The explicit initial bracket profile binds the entire source/STEP/STL/drawing inventory, reruns its bounded checks, and requires separate provenance/interoperability/manufacturability/drawing-PMI/handoff evidence. CAD `prepare_handoff` also forces this gate. Unsupported profiles block; no general CAD validation is inferred. See the [CAD integration and migration contract](../docs/architecture/cad-router-integration.md).
