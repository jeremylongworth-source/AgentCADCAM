# Missing material selection/profile binding
...
The same 2,052-facet revision-A bracket remains measurable at 60 × 40 × 30 mm. A geometrically inspectable file cannot select a material or infer a print process from the STL extension.

## Printer, material and slicer

The job material_id is null. A fixture-pla profile remains present in the input bundle, but no selection binds it to this job. Both profile-identity and compatibility findings must remain visible. Treat this as missing consequential context, not permission to choose a nearby profile.
...
Case-specific unresolved finding: No material is selected and bound to the job, despite the presence of an unrelated candidate profile in the bundle.

Require an explicit material/grade selection and controlled profile with applicable printer, slicer and environmental evidence. Preserve the null selection until that choice is made; do not infer it from the filename, fixture family or candidate profile.
