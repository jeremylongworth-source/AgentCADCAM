...
Removing the final binary STL facet and decrementing its count leaves 2,051 facets with the same 60 × 40 × 30 mm extents. The file checker finds three boundary edges and three nonmanifold vertices even though mesh_status remains valid and this test explicitly binds the changed hash. This is an actual byte-derived defect, not a status-label test. Do not repair it silently or reuse the original derivative's review.

## Printer, material and slicer

Printer and material identities still match the same unverified synthetic profiles. Envelope-size agreement cannot compensate for the open surface. The PrusaSlicer reference experiment also reported three open edges and exported an open mesh successfully; that success did not resolve the defect or grant authority.
...
Case-specific unresolved finding: The derivative is open despite its valid metadata label; manufacturing readiness is blocked independently of the unchanged profile and environmental gaps.

Have the design/geometry reviewer establish a corrected derivative from the controlled source, retain its new identity and derivation evidence, and repeat topology, design-equivalence, slicer and context reviews. Do not infer repair success from a tool's zero exit.
