# 3MF embedded unit conflicts with job/source
...
The package explicitly declares inch, while the source definition and submitted job remain mm. Its numeric 1/1/1 coordinates therefore represent 25.4 mm extents under embedded Core semantics, not the source's 1 mm tetrahedron. Our checker retains the package unit and blocks the declaration conflict even though that larger size still fits the fixture envelope.

## Printer, material and slicer

No evidence authorizes a 25.4-fold scale change. The same unverified printer/material profiles cannot establish design equivalence. The independent PrusaSlicer inch case confirms scale conversion, but its binary STL export has finite-precision rounding and cannot adjudicate intended size.
...
Case-specific unresolved finding: Embedded inch units conflict with the controlled source and job's mm interpretation; envelope fit does not resolve design scale.

Ask the design owner to establish intended physical dimensions and issue a correctly unit-bound package or explicit reviewed conversion. Preserve both conflicting claims until resolved and invalidate dependent reviews after correcting units or package bytes.
