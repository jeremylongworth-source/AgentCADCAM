# 3MF translated build exceeds envelope
...
The tetrahedron has unit extents, but its build transform translates X by 200 mm. Actual transformed bounds are X=200..201, Y=0..1 and Z=0..1; the declared zero-origin 200 mm envelope is exceeded on X. An extent-only test would miss this. PrusaSlicer's local --info bounds are centered, while its exported geometry preserves the translation; use build semantics and actual bytes.

## Printer, material and slicer

Profile identities and unverified states are unchanged. No re-arrangement has been authorized and no actual usable-bed/support clearance has been reviewed. Do not silently recenter the package just because its dimensions are small.
...
Case-specific unresolved finding: The explicit build placement lies outside the submitted printer envelope even though the object's dimensions are small.

Require a reviewed placement for the intended printer, preserve any resulting package hash/revision change and repeat envelope, support/clearance and slicer checks. A reference tool's successful export cannot approve this placement.
