...
The package resolves its model through root relationships and contains one built four-triangle tetrahedron. Core-default millimeter units are explicitly recorded; transformed bounds are 0..1 on each axis and extents are 1/1/1. Partial topology shows one positive-volume shell without a detected defect. The synthetic geometry definition is revision A; the 3MF remains a derivative, not complete product intent.

## Printer, material and slicer

The supplied printer/material profiles are the same unverified test declarations as the STL family. This minimal package includes no actual slicer/printer settings. The absence of unsupported extensions does not create those settings or establish an approved orientation, supports or environment.
...
Case-specific unresolved finding: A parseable Core package with a small build envelope does not establish a complete additive job or applicable profile/environment review.

Preserve the package's whole-file hash and explicit/default unit provenance; obtain applicable printer/material/slicer and job-context evidence, review geometry and intended product requirements, then verify and approve the exact sliced build through qualified review.
