# Measured STL exceeds declared envelope
...
Measured STL extents remain 60 × 40 × 30 mm, but the mutated printer envelope is 50 × 50 × 20 mm. X exceeds the declared envelope by 10 mm and Z by 10 mm; Y alone fits. These observations come from the actual mesh, not the job's dimension label. A speculative rotation, scaling or split is not an authorized correction.

## Printer, material and slicer

Target and material identifiers still match, but their profiles remain synthetic/unverified. Even before usable-bed, support and clearance review, the submitted envelope comparison fails on two axes. The original 200-unit envelope must not be substituted to make this case pass.
...
Case-specific unresolved finding: The submitted 50/50/20 mm build envelope is smaller than the actual mesh on X and Z.

Ask the design/operator reviewers to select and verify an applicable printer/build plan or authorize a controlled design/orientation change. Recheck source intent, all transformed geometry and support/clearance needs; never silently scale the model or raise declared printer limits.
