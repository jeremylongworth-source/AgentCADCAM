# Laser job review — Missing job ventilation context
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/missing-ventilation/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case preserves the original selected drawing bytes; any context mutation is retained explicitly.

## Geometry, units and scale

The unmodified DXF contours and scale remain inspectable. Geometry cannot establish extraction or site conditions.
...
Job ventilation_status is unknown, although the material fixture carries a generic known label. Its source explicitly covers synthetic tests only, not real ventilation safety; it cannot fill the missing job/site evidence.
...
Job-specific ventilation and process-emission context are unresolved.

Require applicable material/process documentation and a qualified site/operator assessment of emission-control requirements and operating conditions. Keep beam review separate; a known beam-control plan would not resolve the missing emission evidence.
