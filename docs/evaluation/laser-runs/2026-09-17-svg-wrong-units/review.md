# Laser job review — Mixed SVG viewport units and displaced geometry
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-wrong-units/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

Only viewport width changes from 70mm to 70in; height remains 50mm. Under the retained default meet alignment, physical extents remain 60 × 40 mm but X bounds shift to 859..919 mm, outside the declared 300 mm working width. This is not a uniform 25.4-fold geometry scaling. The derivative records no single unit because the viewport declarations are mixed.
...
The job/source remain mm and the raw checker reports both unit-declaration conflict and out-of-envelope placement. Matching dimensions alone would miss this case.
...
The mixed-unit SVG viewport conflicts with job units and changes placement despite unchanged geometry extents.

Have the design owner reconcile viewport physical units and intended placement, preserving both conflicting claims. Repeat physical bounds, axis/placement and process reviews for the exact corrected file; do not simply recenter or trust matching extents.
