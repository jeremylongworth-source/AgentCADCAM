...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-positive/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case preserves the original selected drawing bytes; any context mutation is retained explicitly.

## Geometry, units and scale

The revision-A SVG has a 70 × 50 mm viewport with viewBox 0 0 70 50. Its rectangle and two circles measure bounds (5,5)..(65,45) mm and extents 60 × 40 mm. The group id cut is a label, not manufacturing authority.
...
The selected synthetic machine/material/process identities match, but their actual applicability and settings are not established. Numeric agreement with the symmetric DXF fixture does not prove general SVG/DXF orientation or design equivalence.
...
The parseable SVG and matching fixture labels leave real design, path, material, process and site evidence unresolved.

Confirm the controlled design requirements, physical units, axis/orientation and per-path intent; obtain applicable stock/machine/process/site evidence and verify the exact offline output before qualified approval.
