# Laser job review — Unsupported DXF arc
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/unsupported-entity/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

An ARC entity is appended to the actual DXF. The current supported LINE/LWPOLYLINE/CIRCLE reader returns an unsupported-semantics blocker and no complete contour/bounds result. An ARC is not declared malformed merely because this reader cannot evaluate it.
...
The synthetic process profile does not authorize dropping or approximating the added entity. The unchanged supported baseline cannot be substituted for the whole changed file.
...
The changed DXF contains geometry outside the inspected reader subset, so complete drawing interpretation is unresolved.

Request a reviewed applicable reader/adapter or a controlled alternative representation with an explicit geometry/intent comparison. Never strip the arc or reuse the baseline geometry report to make the file pass.

The handoff additionally preserves all unresolved baseline evidence gaps above. Raw preflight blockers: HUMAN_APPROVAL_REQUIRED, MISSING_CONTEXT, SOURCE_VERIFICATION_REQUIRED. Generic router blockers: HUMAN_APPROVAL_REQUIRED, MISSING_CONTEXT, SOURCE_VERIFICATION_REQUIRED. The [handoff](handoff.json) retains their union and [bounded state](state.json); verification stays inconclusive, offline output review is required, and approval is not requested. Raw utility success or schema validity cannot clear any reviewer finding.
