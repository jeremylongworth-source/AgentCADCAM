# Laser job review — Machine/material compatibility conflict
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/machine-material-incompatibility/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case preserves the original selected drawing bytes; any context mutation is retained explicitly.

## Geometry, units and scale

The unchanged DXF still measures 60 × 40 mm. Dimensions alone cannot select a compatible machine or process.
...
The material compatibility object now lists other-laser instead of fixture-laser-40w and contains no process_profiles list. The raw review therefore reports both machine/material and material/process compatibility failures. Matching job/process target IDs do not repair that evidence.
...
The selected machine is excluded from the supplied material compatibility declaration, and process compatibility is also absent.

Obtain a reviewed applicable material/machine/process combination from the responsible reviewers. Do not widen lists, relabel a machine, or invent process settings to bypass the mismatch.
