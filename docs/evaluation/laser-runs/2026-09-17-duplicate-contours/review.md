# Laser job review — Duplicate DXF circle
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/duplicate-contours/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

An additional circle duplicates the hole at (17,25) with radius 3 mm. Four contours are measured, with duplicate_circles=1; bounds remain 60 × 40 mm. The job still declares no duplicates and binds the changed test hash, so the blocker comes from the bytes.
...
The profile identities remain matched synthetic declarations. Envelope fit and unchanged outer dimensions cannot justify retaining the duplicate path.
...
The supplied DXF contains coincident duplicate geometry despite the clean metadata label.

Have the design owner reconcile and issue a controlled drawing with the intended contour multiplicity. Do not silently delete geometry or assume a repeated pass is intended; recheck identity, contour topology, path ordering and all process/site evidence.
