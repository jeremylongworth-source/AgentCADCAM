# Laser job review — Duplicate SVG circle
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-duplicate-contours/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

An extra SVG circle duplicates the hole centered at (17,25), radius 3 mm. The reader reports four contours and duplicate_circles=1 with unchanged 60 × 40 mm bounds. Unchanged clean metadata cannot hide the actual duplicated geometry.
...
Neither the group label cut nor the empty synthetic process profile authorizes multiple coincident passes.
...
The SVG contains a duplicate contour despite an unchanged no-duplicates label.

Reconcile intended contour multiplicity through design/process review and issue a controlled drawing. Do not silently delete a path or infer that a repeated pass is deliberate.
