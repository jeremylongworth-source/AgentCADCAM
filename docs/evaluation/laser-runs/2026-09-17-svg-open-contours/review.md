# Laser job review — Open SVG outer path
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-open-contours/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

The closed rectangle is replaced by a polyline through its four corners without returning to the start. The reader reports one open contour and three outer segments. Bounds remain 60 × 40 mm; the two circles remain closed.
...
The job still declares cut intent and no open contours. A change to score/mark intent would need its own reviewed evidence, not a label change to evade this refusal.
...
The SVG's outer contour is open under the declared cut-path contract.

Request a controlled closed representation or a separately reviewed intent/process contract from the design authority. Do not draw a speculative closing edge or assume the missing segment is harmless.
