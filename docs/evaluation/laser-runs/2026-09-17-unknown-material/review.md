# Laser job review — Unknown material selection
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/unknown-material/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case preserves the original selected drawing bytes; any context mutation is retained explicitly.

## Geometry, units and scale

The original DXF geometry and source identity are unchanged. Partial contour success cannot identify the physical stock or its composition.
...
The job selects unknown-material while the supplied material and process profile still identify fixture-plywood. Both material identity and process target-chain findings remain. Do not borrow the candidate plywood profile.
...
No applicable material identity/grade and matching process chain are established for the selected job.

Ask the job owner to identify the actual stock, grade, thickness and applicable supplier/machine/process evidence. Retain the unknown selection until resolved; do not substitute plywood or guess cutting parameters.
