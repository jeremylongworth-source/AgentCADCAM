# Laser job review — Unsafe material status
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/prohibited-unsafe-material/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case preserves the original selected drawing bytes; any context mutation is retained explicitly.

## Geometry, units and scale

The original DXF still passes only the partial geometry checks. It contains no evidence that can clear material/process safety concerns.
...
The job's unsafe_material_status is unsafe, while the material fixture retains not_indicated. This conflict is not resolved by a verified process flag or known ventilation label. The mutation does not establish that all plywood is unsafe; it tests refusal of a submitted unsafe status.
...
The submitted job explicitly marks the material unsafe; this is a hard stop independent of geometric quality.

Stop this handoff and obtain qualified material/process and site review of applicable primary evidence. Do not clear the flag, substitute a material, alter power/speed, or treat ventilation as permission to process the flagged stock.
