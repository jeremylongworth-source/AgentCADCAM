# Laser job review — Open DXF outer contour
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/open-contours/input.dxf` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

The LWPOLYLINE closed flag changes from 1 to 0. The reader measures one open contour with three outer segments, while the two circles remain unchanged and the bounds remain 60 × 40 mm. The original valid/closed labels do not hide the missing closing edge.
...
The job declares cut intent, not an approved score or marking exception. Matching synthetic profiles cannot supply the missing design decision.
...
The outer cutting contour is open and no reviewed alternate path intent authorizes it.

Require the design authority to provide a corrected closed contour or a separately reviewed process/intent contract. Do not invent a closing segment, relabel the job as marking, or proceed from unchanged profile flags.
