# Laser job review — Unsupported SVG curved path
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-unsupported-entity/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

A cubic C path is appended to the actual SVG. The linear-path reader stops with an unsupported-command blocker; it does not produce a complete contour or bounds result for the changed file.
...
The retained rectangle/circle baseline cannot stand in for the unsupported whole-file content, and no process/profile evidence authorizes dropping or approximating the curve.
...
The actual SVG requires curved-path interpretation outside the inspected reader subset.

Use a reviewed applicable consumer/adapter or obtain a controlled supported alternative with explicit geometry-loss review. Do not remove the curved path or borrow the baseline measurements to clear the blocker.
