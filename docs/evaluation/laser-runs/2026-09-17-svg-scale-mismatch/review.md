# Laser job review — SVG viewBox changes physical scale
...
The [observations](observed.json) preserve the exact submitted job, machine, material and process context, the source inventory hash, selected original drawing hash and derivative hash. The revision-A source inventory is repository-authored synthetic test authority only: it is not authenticated product intent, drawing approval, actual material ownership or permission to manufacture a real job. Source files remain unchanged; `replay:laser/svg-scale-mismatch/input.svg` identifies deterministic in-memory test bytes, not a permanent output file. This case changes drawing bytes and records a new test derivative hash while preserving optimistic geometry/scale labels.

## Geometry, units and scale

The viewBox changes from 0 0 70 50 to 0 0 140 100 while the viewport remains 70 × 50 mm. Physical bounds become (2.5,2.5)..(32.5,22.5), extents 30 × 20 mm and hole radii 1.5 mm. Units and verified scale labels remain mm/verified, but actual size halves.
...
The smaller drawing fits the declared working area; that fact does not establish the intended 60 × 40 design or authorize resizing.
...
The altered viewBox halves physical geometry while the submitted expected design dimensions remain unchanged.

Resolve the intended physical scale with the design authority and retain any corrected file/revision identity. Do not change expected dimensions or silently stretch the viewport to obtain a passing comparison.
