...
The separate schema-instance validator enforces profile source metadata and [lifecycle](../standards/context-profile-standard.md) revision, applicability, units, and verification declarations. Source-field shape validation is not claim verification; see the [source migration](source-contract-migration.md) and [lifecycle migration](profile-lifecycle-migration.md). It also validates the extended [handoff contract](../architecture/handoff-contract.md), including explicit unknowns, review evidence declarations, and status consistency. The [content review](foundation-content-review.md) records Phase 1 acceptance and the downstream evidence-binding work that a passing current-schema run cannot prove.
