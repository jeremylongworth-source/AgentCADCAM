...
The separate schema-instance validator enforces profile source title, publisher, locator, publication availability, access-date format, scope, and supported claims. Source-field shape validation is not claim verification; see the [source migration](source-contract-migration.md). It also validates the extended [handoff contract](../architecture/handoff-contract.md), including explicit unknowns, review evidence declarations, and status consistency. The [content review](foundation-content-review.md) identifies the remaining profile lifecycle schema gap and downstream evidence-binding work that a passing current-schema run cannot prove.
