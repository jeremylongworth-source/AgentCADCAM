...
| 2 / 03: CAD handoff | Five skill contracts; editable bracket source, STEP/STL derivatives, drawing/revision evidence, synthetic provenance mutations, and opt-in native CAD checks. The fixture CLI now includes bounded content validators with actual file-mutation regressions and a retained positive tool report. | Demonstrate skill-assisted handling of all roadmap negative cases with preserved output and review evidence. Fixed declaration/envelope checks do not establish semantic PMI, unchanged-envelope stale-derivative detection, or general source/drawing consistency. See the [CAD evaluation](../evaluation/cad-handoff-evaluation.md). |
