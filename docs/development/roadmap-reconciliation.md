...
| 4 / 05: additive alpha | FDM skill, declared compatibility cases, [STL inspection](../architecture/additive-stl-evidence.md), [bounded 3MF inspection](../architecture/additive-3mf-evidence.md), and an [actual PrusaSlicer/open-model experiment](prusaslicer-windows.md): byte identity, partial topology, Core transforms/units, exact envelope comparisons, retained byte mutations and independent geometry observations. | Unsupported package/vendor semantics, self-intersection/usable-bed limits, retained full skill-assisted handoffs and the complete gate audit remain open. Independent geometry import/export does not establish printable geometry, applicable profiles or an actual sliced build. |
...
1. Complete Phase 4 file-derived FDM geometry/package checks, unit-aware build volume, 3MF distinctions, PrusaSlicer/open-model reference evidence and retained skill outputs. Preserve accepted foundation/CAD/CNC boundaries; do not expand the skill inventory without evaluation evidence.
