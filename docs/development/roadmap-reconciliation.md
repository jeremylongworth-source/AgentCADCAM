...
| 1 / 02: foundation | Seven standards, nine context schemas plus bounded-state schema, local validators, examples, and invalidation tests exist. Nine complete format policies and 11 scoped references are present. Manifest shape/path failures and repository YAML/JSON syntax now have regression coverage. | Skill version/owner/consequence metadata required by the authoring standard is missing. Complete reference-link/anchor validation, content-level contract review, and broader claim evidence remain. See the foundation validation coverage notes; metadata checks do not establish truth or machine applicability. |
...
1. Finish foundation skill metadata, reference coverage, and evidence review. The format/source and malformed-manifest checks are implemented; see [foundation validation coverage](foundation-validation.md) for the remaining limitations. Do not add workflow skills while these remain open.
