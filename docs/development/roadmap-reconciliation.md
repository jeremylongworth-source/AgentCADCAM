...
| 2 / 03: CAD handoff | Five skill contracts; editable bracket source, STEP/STL derivatives, drawing/revision evidence, synthetic provenance mutations, and opt-in native CAD checks. The fixture CLI includes bounded content checks, declared source/derivative byte binding, actual file mutations (including unchanged-envelope cases), and a retained positive tool report. | Demonstrate skill-assisted handling of all roadmap negative cases with preserved output and review evidence. Byte identity is not authenticated derivation truth; semantic PMI and general source/drawing consistency remain unproven. See the [six-case evidence map](../evaluation/cad-handoff-evaluation.md#required-negative-case-evidence-map). |
