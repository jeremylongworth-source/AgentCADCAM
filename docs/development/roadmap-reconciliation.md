...
| 6 / 07: integration | Four-family routing tests; version-2 fingerprints; CNC file/context/verification gates; [laser byte and scoped-evidence composition](../architecture/laser-router-integration.md); local schema checks, stale-approval handling and prohibited live-action blocking. | Complete additive file-aware composition, CAD handoff evidence integration and four-family retained workflow/approval evaluation, then audit the full gate. Current checks establish record consistency, not reviewer identity or truth of supplied evidence. |
