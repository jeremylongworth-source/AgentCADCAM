...
| 7 / 08: hardening | Safety/adversarial tests, approval/IP/regulatory models, security/contributing/limitations documents, and source-freshness process. The [named safety-case matrix](../evaluation/public-alpha-safety-corpus.md) covers all thirteen roadmap cases with 78 distinct adverse boundary checks. [Governance](../architecture/governance-readiness.md) adds source/export refusal; [source assessments](../architecture/source-review-readiness.md) enforce authority declarations, role coverage, metadata binding and UTC freshness for nine context roles. | Complete the remaining factual-source and broader adversarial/public-alpha documentation audit; confirm private reporting availability. The matrix's 100% required-blocker recall and zero false-ready outcomes apply only to its declared structured-input corpus. Declaration consistency does not authenticate permission, source truth or controlled-data classification. |
...
close the complete factual-claim audit.
...
1. Complete the broader public-alpha safety/adversarial review using the named-case map, family regressions and [source/export refusal evidence](../evaluation/governance-runs/README.md). Preserve each corpus's scope and denominator; do not extrapolate structured-input results to arbitrary free-text requests or malicious artifacts.
