...
| 3 / 04: CNC alpha | Seven CNC contracts; curated NC mutations; scoped approval/context checks. The [literal NC reviewer](../architecture/nc-static-review-scope.md) covers compact words, comments, modal/feed/spindle prerequisites, finite numeric limits, unit matching and unsupported-syntax blockers. | Add physical-coordinate-context evidence, bind static findings into composed context/approval review, and retain skill outputs for all required negative cases. Actual feed/spindle behavior, practical independent-tool evidence and gate audit remain unproven. Declared context values are not authenticated machine or simulation evidence. |
...
1. Strengthen CNC static artifact checks and their documented dialect scope; exercise composed job/approval behavior with resulting evidence. Preserve the accepted [foundation boundaries](foundation-validation.md) and [CAD gate limits](cad-handoff-gate-review.md); do not expand the skill inventory without evaluation evidence.
