...
| 2 / 03: CAD handoff | Five skill contracts; editable bracket source and controlled derivatives; restricted source/SVG checks and declared byte binding; file-level negative regressions. The first [retained five-skill run](../evaluation/cad-runs/2026-09-17-manufacturing-intent/review.md) blocks manufacturing handoff for missing required intent despite passing portable/native checks, with a schema-valid state-bound handoff. | Retain and audit skill-assisted outputs for the other five roadmap negative cases, then review all gate criteria. Byte identity is not authenticated derivation truth; general drawing/PMI interpretation and independent practitioner assessment remain outside the current synthetic evidence. See the [six-case map](../evaluation/cad-handoff-evaluation.md#required-negative-case-evidence-map). |
...
1. Complete the remaining five CAD negative-case skill runs with file-derived findings and retained review output, then audit the CAD gate requirements. The missing-manufacturing-intent run is retained but is not an independent practitioner verdict. Foundation acceptance is recorded in the [content review](foundation-content-review.md); preserve its [validation boundaries](foundation-validation.md) and do not expand the skill inventory without evaluation evidence.
