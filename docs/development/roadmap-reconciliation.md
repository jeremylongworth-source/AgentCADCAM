...
| 3 / 04: CNC alpha | Seven CNC contracts; curated NC mutations; scoped approval/context checks. The [literal NC reviewer](../architecture/nc-static-review-scope.md) now covers compact words, comments, selected modal prerequisites and explicit unsupported-syntax blockers. | Add feed/spindle and physical-coordinate-context evidence, bind static findings into composed context/approval review, and retain skill outputs for all required negative cases. Practical independent-tool evidence and gate audit remain open. Declared context values are not authenticated machine or simulation evidence. |
