...
| 1 / 02: foundation | Seven standards, nine context schemas plus bounded-state schema, local validators, examples, and invalidation tests exist. | The format registry lacks explicit per-format revision, suitable/unsuitable workflow, and complete unit policies. Validator success does not establish source authority: the source registry is empty. Audit all required reference and manifest checks against actual failure cases. |
...
| 7 / 08: hardening | Safety/adversarial tests, approval/IP/regulatory models, security/contributing/limitations documents, and source-freshness process. | Populate and verify authoritative claim sources; test unauthorized inputs and export-sensitive handling behavior. Reconcile the complete curated safety corpus before claiming 100% recall or zero critical false-ready states across the roadmap. |
...
1. Close foundation policy and evidence gaps: complete the format entries, verify authoritative sources against the claims actually used, and test required validator failure modes. Do not add workflow skills while these remain open.
