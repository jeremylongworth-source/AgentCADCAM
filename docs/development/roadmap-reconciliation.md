...
| 4 / 05: additive alpha | FDM skill, [STL inspection](../architecture/additive-stl-evidence.md), [bounded 3MF inspection](../architecture/additive-3mf-evidence.md), an [actual PrusaSlicer experiment](prusaslicer-windows.md), and [twelve retained skill-assisted handoffs](../evaluation/additive-runs/README.md). File-derived failures, all seven required negatives, unit/placement/extension distinctions and unresolved profile/slicer/environment evidence are preserved. | The complete exit-gate audit must reconcile supported behavior and remaining package/vendor, self-intersection and usable-bed boundaries. Retained reviews are same-agent synthetic evidence, not applicable profile verification, a sliced build or practitioner approval. |
...
1. Audit Phase 4 against every required responsibility, negative case and exit criterion, using the file-derived STL/3MF checks, unit-aware envelope comparisons, PrusaSlicer observations and retained skill-assisted handoffs. Reconcile unresolved geometry/package limits and any acceptance counterexamples before awarding gate 05. Preserve accepted foundation/CAD/CNC boundaries; do not expand the skill inventory without evaluation evidence.
2. Complete Phase 5 laser geometry/scale mutation checks against actual files and retain applicable skill outputs, not only pre-labeled metadata cases.
