...
| 3 / 04: CNC alpha | Seven CNC contracts; curated NC mutations; scoped approval/context checks. [Composed routing](../architecture/nc-artifact-approval-binding.md) binds actual bytes and reruns literal/modal/feed/spindle review. The [coordinate model](../architecture/nc-coordinate-model.md) checks declared translation/initial-position/target bounds. [Retained reviews](../evaluation/cnc-runs/README.md) cover all seven skills for baseline and eight negatives. An [actual CAMotics experiment](../architecture/camotics-evidence-boundary.md) independently interprets those bytes and exports synthetic cut-prism surfaces, demonstrating defaults/acceptance that must not override review gates. | The full gate audit remains open. Declared mapping and synthetic simulation do not prove physical state, collision clearance, real feed/spindle behavior or applicable job simulation. Retained agent outputs are not practitioner verdicts. Multi-tool static semantics remain unsupported and block this path; CAMotics auto-created tools cannot supply that missing evidence. |
