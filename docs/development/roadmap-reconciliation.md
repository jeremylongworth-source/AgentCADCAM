...
| 3 / 04: CNC alpha | Seven CNC contracts; curated NC mutations; scoped approval/context checks. The literal NC reviewer covers selected modal/feed/spindle and numeric checks. [Composed routing](../architecture/nc-artifact-approval-binding.md) now requires actual bytes, checks state-bound identity and reruns static review before recognizing an execution-adjacent CNC approval. | Add physical-coordinate-context evidence and retain skill outputs for all required negative cases. Actual feed/spindle behavior, simulation truth, practical independent-tool evidence and gate audit remain unproven. Multi-tool static semantics are not implemented and must block this path. |
