...
| 6 / 07: integration | Four-family routing tests; version-2 fingerprints; CNC file/context/verification gates; [laser](../architecture/laser-router-integration.md), [additive](../architecture/additive-router-integration.md) and [bounded CAD](../architecture/cad-router-integration.md) byte/scoped-evidence composition; [serialized handoff/state binding](../architecture/handoff-state-binding.md); [four retained integrated skill reviews and 24 separate approval-lifecycle controls](../evaluation/integration-runs/README.md); stale-approval handling and prohibited live-action blocking. | Audit the full gate against its deliverables and exit criteria. The retained reviews preserve actual fixture gaps and blocked handoffs; separate synthetic controls establish scoped file/record consistency, not general CAD interpretation, reviewer identity or truth of supplied evidence. |
...
1. Audit Phase 6 composed file-aware workflows and current-context approval/invalidation behavior against every roadmap deliverable and exit criterion, using the new integrated packets and isolated lifecycle controls as evidence. Preserve accepted development boundaries; do not expand the skill inventory without evaluation evidence.
