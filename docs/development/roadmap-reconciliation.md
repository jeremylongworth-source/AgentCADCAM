...
| 4 / 05: additive alpha | FDM skill, declared compatibility cases, [actual STL inspection](../architecture/additive-stl-evidence.md), and [bounded 3MF inspection](../architecture/additive-3mf-evidence.md): byte identity, partial topology, Core units/resources/build transforms, exact extent/rectangular placement comparisons and retained byte mutations. | Independent PrusaSlicer/open-model reference evidence, unsupported package/vendor semantics, self-intersection/usable-bed limits and retained skill-assisted handoffs remain open. Partial topology and rectangular envelope checks do not establish printable geometry or an actual sliced build. |
