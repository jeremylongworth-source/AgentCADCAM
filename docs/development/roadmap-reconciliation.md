...
| 1 / 02: foundation | Seven standards, nine context schemas plus bounded-state schema, local validators, examples, and invalidation tests exist. All nine format entries now include the eight roadmap policy fields; 11 scoped format references and offline completeness/citation checks are present. | Format policy omissions and the empty source registry have been addressed. Audit the remaining required reference/manifest validator failure cases and broader claim evidence. Source metadata checks do not establish truth or machine applicability. |
...
1. Finish foundation evidence and validator failure-mode coverage. The format fields and scoped format references are now populated; broader source and manifest/reference checks remain. Do not add workflow skills while these remain open.
