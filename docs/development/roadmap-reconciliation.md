...
| 4 / 05: additive alpha | FDM skill, declared compatibility cases and [actual STL inspection](../architecture/additive-stl-evidence.md): byte identity, partial topology, real missing-facet observations and declared-unit measured extent comparison. | 3MF package semantics, self-intersection/placement limits, PrusaSlicer/open-model reference evidence and retained skill-assisted handoffs remain open. Partial topology and axis-aligned extent fit do not establish printable geometry or an actual sliced build. |
