...
| 5 / 06: laser alpha | Laser skill; [actual DXF/SVG contour and byte-aware preflight](../architecture/laser-file-evidence.md); altered-file geometry/unit/scale cases, explicit source/hash/placement checks and material/process/environment failures independent of the human-approval flag. | Complete retained skill-assisted handoffs and the gate audit remain. Partial geometry and declaration consistency do not establish path intent, design equivalence, applicable process/site evidence or manufacturing approval. Full router/state composition remains Phase 6 work. |
...
1. Retain complete Phase 5 laser skill-assisted outputs using the actual DXF/SVG geometry/scale and context failures, then audit every responsibility and exit criterion. Preserve accepted foundation/CAD/CNC/additive boundaries; do not expand the skill inventory without evaluation evidence.
