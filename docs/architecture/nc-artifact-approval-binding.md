...
4. If artifact identity and required context are usable, decode strict UTF-8 and run the [static reviewer](nc-static-review-scope.md) against the same state machine, controller, setup, post and tool profile. Pass the selected job WCS explicitly rather than assuming the controller's first supported WCS. The current one-tool reviewer rejects a library with multiple tools; it does not quietly select one.
...
The standalone `review_program` function remains an evidence utility, not an approval evaluator. Its optional `selected_wcs` parameter lets callers supply the actual selected code; the original fixture-only fallback remains for its standalone tests. The composed router always supplies the selected code.
