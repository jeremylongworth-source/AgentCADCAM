...
`route_job` evaluates bounded state and approval records; it does **not** consume this serialized handoff or authenticate its evidence. The subsequent [`review_handoff` consumer](handoff-state-binding.md) compares a package with state-bound review details, current process checks and scoped approval. It preserves old fingerprints and returns blocked/invalidated copies on conflicts, never automatic approval. Retained integrated workflow evaluation and practitioner validation remain open. Do not treat schema validity or record consistency as physical operating authority.
